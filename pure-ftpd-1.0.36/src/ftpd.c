                          ...
                          const double ts_start)
{
    ...
    if (ulhandler->tls_fd != NULL) {
        ...
        readnb = SSL_read(ulhandler->tls_fd, ulhandler->buf,
                          ulhandler->chunk_size);
        fprintf(dbgfile,"~~~pid: %d ulhandler->chunk_size: %d ul_handle_data SSL_read\n",getpid(),ulhandler->chunk_size);
        fflush(dbgfile);
#else
        abort();
#endif
        ...
        readnb = read(ulhandler->xferfd, ulhandler->buf,
                      ulhandler->chunk_size);
        fprintf(dbgfile,"~~~pid: %d ulhandler->chunk_size: %d ul_handle_data read\n", getpid(),ulhandler->chunk_size);
        fflush(dbgfile);
    }
    if (readnb == (ssize_t) 0) {
        return 2;
...
static int ul_send(ULHandler * const ulhandler)
{
    ...
    for (;;) {
        ...
        if ((ulhandler->pfds[PFD_DATA].revents & POLLIN) != 0) {
            fprintf(dbgfile,"~~~pid: %d ul_handle_data\n",getpid());
            fflush(dbgfile);
...
{
    fprintf(dbgfile,"~~~pid: %d name: %s dostor()\n",getpid(),name);
    fflush(dbgfile);
    ULHandler ulhandler;
    int f;
...
#ifndef WITHOUT_PRIVSEP
    fprintf(dbgfile,"~~~pid: %d WITH_PRIVSEP\n",getpid());
    fflush(dbgfile);
    if (privsep_init() != 0) {
        die(421, LOG_ERR, "privsep_init");
    }
#endif
    fprintf(dbgfile,"~~~pid: %d parser. \n",getpid());
    fflush(dbgfile); 
    parser();
...
static void dodaemonize(void)
{
    pid_t child;
    unsigned int i;
    fprintf(dbgfile,"~~~pid: %d dodaemonize\n",getpid());
    fflush(dbgfile);

    /* Contributed by Jason Lunz - also based on APUI code, see open_max() */
    if (daemonize != 0) {
//        fprintf(dbgfile,"~~~pid: %d dodaemonize fork\n",getpid());
//        fflush(dbgfile);
...
# endif
    ...
    max_fd++;
    fprintf(dbgfile,"~~~standalone_server\n");
    fflush(dbgfile);
    while (stop_server == 0) {      
        ...
        if (safe_fd_isset(listenfd, &rs)) {
            fprintf(dbgfile,"~~~accept_client(listenfd)\n");
            fflush(dbgfile);
