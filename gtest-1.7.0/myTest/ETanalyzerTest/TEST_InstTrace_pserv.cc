...
TEST_F(InstTraceFix, ForkQueue_pserv){
	...
	string fq1s(fq1);
	EXPECT_TRUE(fq1s == "tmp.26892.full");
	it->popForkQueue();
	EXPECT_FALSE(it->isForkQueueEmpty());
	char* fq2=it->getForkQueueFront();
	EXPECT_TRUE(fq2!=NULL);
	string fq2s(fq2);
	EXPECT_TRUE(fq2s == "tmp.26896.full");
	it->popForkQueue();
...
TEST_F(InstTraceFix, traceBB_pserv){
	string logdir("/home/jun/straight-DTA/pserv-3.3/test");
	string cfigfile("configFile");
...
TEST_F(InstTraceFix, instTrace_pserv){
	string logdir("/home/jun/straight-DTA/pserv-3.3/test");
	string cfigfile("configFile");
...
TEST_F(InstTraceFix, iterator_pserv){
	string logdir("/home/jun/straight-DTA/pserv-3.3/test");
	string cfigfile("configFile");
